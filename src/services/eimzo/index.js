...
export const loadKey = (itemObject, success, fail, verifyPassword) => {
  if (itemObject) {
    var vo = itemObject;
...
export const createPkcs7 = (id, data, timestamper, success, fail) => {
  CAPIWS.callFunction(
    {
      plugin: 'pkcs7',
      name: 'create_pkcs7',
      arguments: [btoa(unescape(encodeURIComponent(data))), id, 'no'],
    },
    function (event, data) {
      console.log(data);
      if (data.success) {
...
export const appendPkcs7Attached = (id, data, timestamper, success, fail) => {
  CAPIWS.callFunction(
    {
      plugin: 'pkcs7',
  ...
  new Promise(async (resolve, reject) => {
    if (!cert) {
      try {
        ...
        <Alert
          message={i18next.t('Xatolik yuz berdi!')}
          type="warning"
          showIcon
          closable
        />;
    ...
    try {
      ...
      <Alert message={i18next.t('Xatolik yuz berdi!')} type="warning" showIcon closable />;
      reject();
    }
  });
      ...
      default:
        resolve(
          filteredCerts.sort((a, b) =>
            parseValidDate(a.parsedAlias.validto) > parseValidDate(b.parsedAlias.validto)
              ? -1
              : 1
          )[0]
        );
...
export const getAllCertificates = (uid) =>
  new Promise((resolve, reject) =>
    Promise.all([getAllCertificatesPfx()])
      .then((certificates) => {
        const certs = [...parseData(certificates[0].certificates, 'pfx')];
        if (uid) {
